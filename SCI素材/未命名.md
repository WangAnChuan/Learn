...
所有实验均在搭载 Ubuntu 22.04 与单张 **NVIDIA RTX 5090 GPU** 的工作站上基于 PyTorch 框架完成。我们选用 **YOLO11n** 作为基准模型并加载 COCO 预训练权重进行迁移学习，输入分辨率统一为 $640 \times 640$，批次大小（Batch Size）设为 32。训练全过程持续 200 个 Epochs，采用 **SGD 优化器**（初始学习率 0.01，动量 0.937，权重衰减 $5 \times 10^{-4}$）并开启自动混合精度（AMP）加速；为消除坐标漂移并微调 BatchNorm 统计量，我们在训练的最后 10 个 Epoch **强制关闭 Mosaic 数据增强**。
